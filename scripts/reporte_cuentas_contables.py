...
<div class="container">
    ...
    <section class="section">
        ...
        <div class="filters">
            ...
            <button id="clear-date-filters" type="button">Limpiar fechas</button>
        </div>
        <p>El rango de fechas filtra estos registros y actualiza también el total por cuenta contable y el resumen por flujo.</p>
        <div class="filters">
            <label>Cuenta contable
                <select id="account-filter"><option value="">Todas</option></select>
            </label>
            <label>Estado
                <select id="status-filter"><option value="">Todos</option><option value="Con cuenta contable">Con cuenta contable</option><option value="Sin cuenta contable">Sin cuenta contable</option></select>
            </label>
            <label>Flujo
...
(function () {{
    ...
    function applyDetailFilters() {{
        ...
        rows.forEach(function (row) {{
            const matches = inDateRange(row.dataset.date)
                && (!account || row.dataset.accountid === account)
    ...
    rows.forEach(function (row) {{
        if (row.dataset.accountid && !accountOptions.has(row.dataset.accountid)) accountOptions.set(row.dataset.accountid, row.dataset.account);
    }});
    Array.from(accountOptions.entries()).sort(function (left, right) {{ return left[1].localeCompare(right[1]); }}).forEach(function (entry) {{
    ...
    [accountFilter, statusFilter, flowFilter, clientFilter, referenceFilter].forEach(function (input) {{
